...
function Dashboard({ user }) {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true)
      const [cases, docsResponse] = await Promise.all([
        listCases(100, 0),
      ...
      const completedThisWeek = casesList.filter(c => {
      ...
      const activity = casesList.slice(0, 4).map((c, i) => {
        const d = c.created_at || c.timestamp
        const timeAgo = d ? getTimeAgo(new Date(d)) : 'Unknown'
        return {
          id: c._id || i,
          action: `Case analysis: ${(c.case_text || '').substring(0, 30)}...`,
          time: timeAgo,
          type: 'case'
        }
      })
      ...
      setStats({ totalCases: 0, activeCases: 0, documentsUploaded: 0, completedThisWeek: 0 })
      setRecentActivity([])
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-8">
      <div className="mb-8 text-center">
        <img src={lawLogo} alt="Law Logo" className="h-16 w-16 mx-auto mb-4 hover:scale-110 transition-transform duration-300" />
        <h1 className="text-3xl font-bold text-white mb-2">
          Welcome back, {user?.email?.split('@')[0] || 'User'}!
        </h1>
        <p className="text-lg text-gray-300">Here's what's happening with your legal work today</p>
      </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-gray-800 rounded-lg shadow p-6 border border-gray-700 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Link to="/cases" className="bg-green-600 hover:bg-green-700 text-white p-4 rounded-lg transition-colors duration-200 flex flex-col items-center text-center">
                  ...
                  <Link to="/documents" className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-lg transition-colors duration-200 flex flex-col items-center text-center">
                    <svg className="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                    </svg>
                  ...
                  <Link to="/cases" className="bg-purple-600 hover:bg-purple-700 text-white p-4 rounded-lg transition-colors duration-200 flex flex-col items-center text-center">
                    <svg className="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
